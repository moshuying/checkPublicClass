...
try {
    if($_FILES["file"]["error"]){
        ...
        if(($_FILES["file"]["type"]=="image/png"||$_FILES["file"]["type"]=="image/jpeg"||$_FILES["file"]["type"]=="image/gif")&&$_FILES["file"]["size"]<1024000){
             $filename ="classImages/".time().rand(1,100);//防止文件名重复
            ...
            if(file_exists($filename)){//检查文件或目录是否存在
                ...
                if(move_uploaded_file($_FILES["file"]["tmp_name"],$filename)){//将临时地址移动到指定地址    
                    ...
                    if(empty($title)||empty($author)||empty($miaoshu)||empty($neirong1)||empty($preparation)){
                        ...
                        $addClassSql="UPDATE `lesson` SET `title`='$title',`author`='$author',`miaoshu`='$miaoshu',`preparation`='$preparation',`neirong1`='$neirong1',`time1`='$time1',`neirong2`='$neirong2',`time2`='$time2',`neirong3`='$neirong3',`time3`='$time3',`neirong4`='$neirong4',`time4`='$time4',`number`='$number',`logoSource`='$logoSource' WHERE (`ID`='$ID')";
                        $addClassSqlInsert=$pdo->exec($addClassSql);
                        header("Location:../admin/MAddClass.php?submit2=1");
                    }
                }else{ 
                    header("Location:../admin/MAddClass.php?submit2=4");
                }
            }
        }else{
            header("Location:../admin/MAddClass.php?submit2=4");
