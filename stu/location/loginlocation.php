    ...
    if(!empty($username)&&!empty($password)){
        if($username[0]=="0"){
            ...
            if($username==$tealoginQuery['jobNumber']&&$password==$tealoginQuery['teaPassword']){
                ...
                header("Location:../teaCheckClass.php");
            }else {
            //用户名或密码错误，赋值err为1
        ...
        if($username[0]=='2'){
            //第一位为2 检查学生账号    
            $loginSql="SELECT username,password,xingming,xuankeID FROM studentxk WHERE username = ".$username; 
            //执行sql语句
            ...
            if($username==$result['username']&&$password==$result['password']){
                ...
                $pdo -> exec($sql_logs);
                usleep(500);//页面跳转太快,会出现数据还未写完页面已经发生跳转,暂停0.5秒等到数据写完再进行下一步操作
                header("Location:../stuCheckClass.php");
            }else {
            //用户名或密码错误，赋值err为1
            header("Location:../login.php?err=1");
            }
        ...
        if($username[0]=='6'){
            ...
            if($username==$tealoginQuery['MID']&&$password==$tealoginQuery['Mpassword']){
                ...
                $pdo -> exec($sql_logs);
                usleep(500);//页面跳转太快,会出现数据还未写完页面已经发生跳转,暂停0.5秒等到数据写完再进行下一步操作
                header("Location:../MCheckClass.php");
            }else {
            //用户名或密码错误，赋值err为1
            header("Location:../login.php?err=1");
            }
