        ...
        if (
          ...
          for (var ix in JSONData) {
            var NoticeStr =
              ...
              JSONData[ix].info +
              "</p></br>";
            console.log(JSONData[ix].uptime + " " + JSONData[ix].updateName);
        ...
        if (
          ...
          if (jsonDATA.length == 0 || jsonDATA == null) {
            ...
            var ShowHeat = jsonDATA[ix].selected + 20 + jsonDATA[ix].clickOn + 10,
              ...
              ');height: 200px;background-size: cover;"></div><p style="cursor: pointer;" onclick="$(\'#jumpClick' + ix + '\').click();" class="button"> <span class="icon icon-play"> </span></p></aside><article><h2 id="jumpClick' + ix + '" onclick="signIn(this)">' +
              jsonDATA[ix].title +
